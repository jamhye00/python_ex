...
fruits = ['사과', '포도', '복숭아']      # 컨테이너 자료형: list(리스트) # 변수명을, 복수명으로 한다
print(f'fruits: {fruits}')
print(f'fruits type: {type(fruits)}')

# 파이썬에서 컨테이너 자료형으로는 list(리스트), tuple(튜플), dictionary(딕셔너리) 가 있다.

# list(리스트) 정의(선언 + 초기화)
fruits = ['사과', '포도', '복숭아']

# index(인덱스): 아이템에 부여된 아이템 식별 번호
#   0       1       2
#['사과', '포도', '복숭아']
...
# 리스트의 전체 데이터 조회
# 리스트는 반복 가능한 객체(데이터)입니다. ==> 이터너블한 데이터
for fruit in fruits:
    print(f'fruit: {fruit}')   
...
# pop()과 del의 차이점
nums = [1, 2, 3, 4, 5, 6]
deleteNum = nums.pop(3)     # [1, 2, 3, 5, 6]           # deleted item: 4 을 받아낼 수 있다. del은 안됨
print(f'deletedNum: {deleteNum}')
...
이 중 과일이 아닌 '당근'과 '토마토'를 찾아 삭제하는 프로그램을 만들어봅시다.
'''
...
for item in fruits:
    if item == '당근' or item == '토마토':
        fruits.remove(item)
print(f'fruit: {fruits}')               # ['사과', '망고', '수박', '포도', '참외']
